...
def execute_sql(sql, params=None, fetch=False, fetchall=False):
    try:
...
def home():
    ...
    query = "SELECT USER_NOME, USER_ID FROM USUARIOS WHERE USER_EMAIL = :email"
    ...
    # Buscar os remédios associados ao USER_ID do usuário logado
    query_medicamentos = "SELECT REMEDIO_NOME, REMEDIO_DATA, REMEDIO_HORA FROM REMEDIOS WHERE USER_ID = :user_id"
    medicamentos = execute_sql(query_medicamentos, {"user_id": user_id}, fetchall=True)

    return render_template('index.html', user_name=user_name, medicamentos=medicamentos)
...
def cadastrar_remedio():
    ...
    usuario = execute_sql(query, {'email': user_email}, fetch=True)
